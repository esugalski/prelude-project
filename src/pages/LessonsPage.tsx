...
import { ArrowRight, CheckCircle2, Music2, BookOpen, Plus, Trash2 } from 'lucide-react';
...
export function LessonsPage() {
  ...
  useEffect(() => {
    if (user?.email) {
      setParent((p) => ({ ...p, parent_email: user.email! }));
    }
  }, [user]);
  ...
  const updateChild = (index: number, field: string, value: string | number) =>
    setChildren((cs) => cs.map((c, i) => (i === index ? { ...c, [field]: value } : c)));

  const toggleChildInstrument = (index: number, opt: string) =>
    setChildren((cs) =>
      cs.map((c, i) => {
        if (i !== index) return c;
        const current = c.instruments.includes(opt)
          ? c.instruments.filter((x) => x !== opt)
          : [...c.instruments, opt];
        ...
        if (opt === 'Not sure yet' && current.includes(opt)) {
          ...
          instruments = current.filter((x) => x !== 'Not sure yet');
        }
        if (instruments.length === 0) instruments = [instrumentOptions[0]];
        return { ...c, instruments, instrument_interest: instruments.join(', ') };
      })
    );
  ...
  const removeChild = (index: number) =>
    setChildren((cs) => (cs.length > 1 ? cs.filter((_, i) => i !== index) : cs));

  const ageIssues = children.map((c) => {
    const n = Number(c.child_age);
    return c.child_age !== '' && (isNaN(n) || n < 5 || n > 13);
  });

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    const invalidAge = children.some((c) => {
      const n = Number(c.child_age);
      return isNaN(n) || n < 5 || n > 13;
    });
    if (invalidAge) {
      setError('Our program is for children ages 5–13. Please enter an age in that range for each child.');
      return;
    }
    setSubmitting(true);
    setError('');
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('lesson_enrollments').insert(rows);
  ...
  if (done) {
    const firstNames = children.map((c) => c.child_name.split(' ')[0]).filter(Boolean);
    const namesText =
      firstNames.length <= 1
        ? firstNames[0] ?? 'your child'
        : firstNames.length === 2
      ...
      <section className="mx-auto max-w-2xl px-6 pt-40 pb-24 text-center">
        <Reveal>
          ...
          <h1 className="mt-6 font-display text-4xl tracking-tight">
            {children.length > 1 ? 'Your children are on the list.' : 'Your child is on the list.'}
          </h1>
          <p className="mt-4 text-foreground/70">
            Thank you, {parent.parent_name.split(' ')[0]}. We've received your enrollment for{' '}
            {namesText}. Our coordinator will contact you at{' '}
            {parent.parent_email} to match {children.length > 1 ? 'them' : 'your child'} with{' '}
            {children.length > 1 ? 'teachers' : 'a teacher'} and arrange their first lesson{children.length > 1 ? 's' : ''}.
          </p>
          ...
          <Reveal>
            ...
            <div className="mt-8 p-5 bg-accent/30 rounded-sm border border-accent">
              <p className="text-accent-foreground leading-relaxed text-sm">
                ...
                a time that works for your family. Enrolling more than one child? Just add them below —
                one form covers your whole family.
              </p>
            </div>
          </Reveal>
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateParent('parent_email', e.target.value)}
              ...
              <div className="space-y-5">
                {children.map((child, i) => (
                  <div key={i} className="relative border border-border rounded-sm p-5 space-y-4 bg-card/20">
                    <div className="flex items-center justify-between">
                      ...
                      {children.length > 1 && (
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 text-xs font-medium text-foreground/50 hover:text-destructive transition-colors"
                        >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateChild(i, 'child_name', e.target.value)}
                        className="mt-1.5 w-full px-4 py-3 border border-input rounded-sm bg-background focus:outline-none focus:ring-2 focus:ring-ring"
                      />
                    </div>
              ...
              <button
                type="button"
                onClick={addChild}
                className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:gap-2.5 transition-all"
              >
              ...
              >
                {submitting ? 'Submitting...' : children.length > 1 ? 'Enroll my children' : 'Enroll my child'}
